...
parser = argparse.ArgumentParser(description="REPORT generator for ICUCamera RAW files for peopel at South Pole")
parser.add_argument("--input", type=str, help="input raw file with full path", required=True)
parser.add_argument("--outputdir", type=str, help="output directory", default="./")
args = parser.parse_args()

fname = args.input
rawname = fname.split("/")[-1]
output = os.path.join(args.outputdir, rawname.replace(".raw", "_report.pdf"))
